...
class KiloAgentBackend extends AgentBackend {
  constructor(options = {}) {
    super();
    this.serverUrl = options.serverUrl || process.env.KILO_SERVER_URL || 'http://127.0.0.1:4097';
  ...
  send(options) {
    ...
    this.log('send() starting', { sessionId, model, mode, promptLength: prompt?.length || 0 });
