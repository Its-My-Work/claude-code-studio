...
Config files in ~/.config/kilo/: use kilo.jsonc for provider, model, permission, and MCP settings. Restart the CLI after editing. See Using MCP in Kilo Code for MCP config format.
...
💡
Tip
Confused about /newtask vs /smol in the IDE? See the Using Agents documentation for details.
...
Add the pattern to your execute.allowed list in the config
Auto-approve matching commands in the future
