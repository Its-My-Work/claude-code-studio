...
console.log('real request -> each dialect:');
{
  ...
  check('mistral: no field its strict validator rejects (user, stream_options)', ['user' in ms, 'stream_options' in ms, ms.stream], [false, false, true]);
  check('gemini: no user field', 'user' in gm, false);
...
console.log('stream: mid-stream error:');
{
  ...
  const orr = runStream([ch({ content: 'partial' }), { error: { code: 502, message: 'Provider disconnected' }, choices: [{ index: 0, delta: { content: '' }, finish_reason: 'error' }] }]);
  check('OpenRouter error chunk (error + choices) is an error, not end_turn', [streamProblems(orr, { expectError: true }), orr[orr.length - 1][1].error.message], [[], 'Provider disconnected']);
  const fe = runStream([ch({ content: 'partial' }), fin('error')]);
  check('finish_reason "error" alone is an error too', streamProblems(fe, { expectError: true }), []);
