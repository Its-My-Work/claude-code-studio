...
- **A test server that drives runs through a fake headless `claude` spreads `test/_api-provider-env.js` into its env.** With no gateway the default provider is the CLI login, and in CI (tmux installed, no `ANTHROPIC_BASE_URL`) its models go to the tmux engine — the suite then passes on a developer box whose shell exports `ANTHROPIC_BASE_URL` and fails only in CI.
...
### A spawn command tmux refuses to carry (issue #96)

A tmux COMMAND travels to the tmux server over its imsg socket, which caps the whole
message at ~16 KB. Measured on tmux 3.7c: a 16 300-byte `new-session` command starts,
16 340 answers `failed to send command`, 20 000 `command too long`. The subscription
engine put the entire system prompt inside that command, so a project with a 25 KB
`AGENTS.md` — legal, `agents-md.MAX_BYTES` is 64 KB — could never start an interactive
session.

- **The prompt travels as a FILE PATH, never inline.** `tmuxLaunchCommand()` writes the
  whole invocation to a script and hands tmux `sh <path>`: ~60 bytes whatever the prompt
  weighs. The ceiling that then applies is ARG_MAX (~1 MB macOS, ~2 MB Linux), which is
  what `agents-md.MAX_BYTES` was always sized against.
- **A script, not `--append-system-prompt "$(cat file)"`.** tmux runs the command through
  `default-shell` — the user's LOGIN shell. `$( )` is a syntax error in csh/tcsh, and
  everywhere else it strips trailing newlines. `sh <path>` is plain words in every shell,
  and the script's own interpreter is one we choose. The script `exec`s, so the pane's
  process is still `claude`.
- **The scripts live in a per-process 0700 `mkdtemp` directory, not on a shared /tmp
  path.** They are content-addressed and reused on respawn, and this file is EXECUTED —
  a predictable name in a world-writable directory is pre-creatable by another local
  user, which would run their script instead. `mcpConfigPath()` is data, not code, which
  is why it can stay where it is.
- **Failing to write the script falls back to the pre-#96 inline command.** A read-only
  tmpdir then behaves exactly as before rather than losing the engine entirely.
- **tmux stderr is PIPED, not ignored.** `command too long` was the entire content of the
  bug and it was discarded at the spawn, leaving one generic line on screen. It is now
  appended to the `error` frame.
