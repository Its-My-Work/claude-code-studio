...
const BOT_I18N = {
  uk: {
    ...
    'bots_btn_topic': '🤖 Топік: {name}',
    'bots_btn_capped': 'ℹ️ Кнопки показано для перших {shown} з {total} ботів.',
    'forum_topic_bot': '🤖 <b>{name}</b>\n\nЦе топік бота <code>@@{handle}</code>. Усе, що ви тут напишете, адресується саме йому — звертання дописувати не треба.\n\n📁 <code>{path}</code>',
    'forum_bot_topic_exists': 'ℹ️ У цього бота вже є топік: {link}',
    'forum_bot_topic_no_project': '⚠️ Топік бота створюється з топіка проєкту — перейдіть у нього і повторіть.',
    'forum_bot_topic_gone': '⚠️ Такого бота більше немає в ростері цього проєкту.',
    'forum_bot_topic_failed': '❌ Не вдалося створити топік. Перевірте, чи має бот право керувати темами.',
  ...
  en: {
    ...
    'bots_btn_topic': '🤖 Topic: {name}',
    'bots_btn_capped': 'ℹ️ Buttons shown for the first {shown} of {total} bots.',
    'forum_topic_bot': '🤖 <b>{name}</b>\n\nThis is <code>@@{handle}</code>\'s own topic. Everything you write here goes to that bot — no need to type the handle.\n\n📁 <code>{path}</code>',
    'forum_bot_topic_exists': 'ℹ️ This bot already has a topic: {link}',
    'forum_bot_topic_no_project': '⚠️ A bot topic is created from a project topic — go to one and try again.',
    'forum_bot_topic_gone': '⚠️ That bot is no longer in this project\'s roster.',
    'forum_bot_topic_failed': '❌ Could not create the topic. Check that the bot may manage topics.',
  ...
  ru: {
    ...
    'bots_btn_topic': '🤖 Топик: {name}',
    'bots_btn_capped': 'ℹ️ Кнопки показаны для первых {shown} из {total} ботов.',
    'forum_topic_bot': '🤖 <b>{name}</b>\n\nЭто топик бота <code>@@{handle}</code>. Всё, что вы здесь напишете, адресуется именно ему — обращение дописывать не нужно.\n\n📁 <code>{path}</code>',
    'forum_bot_topic_exists': 'ℹ️ У этого бота уже есть топик: {link}',
    'forum_bot_topic_no_project': '⚠️ Топик бота создаётся из топика проекта — перейдите в него и повторите.',
    'forum_bot_topic_gone': '⚠️ Такого бота больше нет в ростере этого проекта.',
    'forum_bot_topic_failed': '❌ Не удалось создать топик. Проверьте, есть ли у бота право управлять темами.',
  ...
  fr: {
    ...
    'bots_btn_topic': '🤖 Sujet : {name}',
    'bots_btn_capped': 'ℹ️ Boutons affichés pour les {shown} premiers bots sur {total}.',
    'forum_topic_bot': '🤖 <b>{name}</b>\n\nCeci est le sujet de <code>@@{handle}</code>. Tout ce que vous écrivez ici lui est adressé — inutile de retaper l\'identifiant.\n\n📁 <code>{path}</code>',
    'forum_bot_topic_exists': 'ℹ️ Ce bot a déjà un sujet : {link}',
    'forum_bot_topic_no_project': '⚠️ Un sujet de bot se crée depuis un sujet de projet — allez-y et réessayez.',
    'forum_bot_topic_gone': '⚠️ Ce bot ne fait plus partie du roster de ce projet.',
    'forum_bot_topic_failed': '❌ Impossible de créer le sujet. Vérifiez que le bot peut gérer les sujets.',
  ...
  he: {
    ...
    'bots_btn_topic': '🤖 נושא: {name}',
    'bots_btn_capped': 'ℹ️ הכפתורים מוצגים עבור {shown} הבוטים הראשונים מתוך {total}.',
    'forum_topic_bot': '🤖 <b>{name}</b>\n\nזהו הנושא של <code>@@{handle}</code>. כל מה שתכתבו כאן מופנה אליו — אין צורך להקליד את הכינוי.\n\n📁 <code>{path}</code>',
    'forum_bot_topic_exists': 'ℹ️ לבוט הזה כבר יש נושא: {link}',
    'forum_bot_topic_no_project': '⚠️ נושא של בוט נוצר מתוך נושא של פרויקט — עברו לשם ונסו שוב.',
    'forum_bot_topic_gone': '⚠️ הבוט הזה כבר לא נמצא ברשימת הפרויקט.',
    'forum_bot_topic_failed': '❌ לא הצלחנו ליצור את הנושא. בדקו שלבוט יש הרשאה לנהל נושאים.',
